...
const FALLBACK_THUMB_PLACEHOLDER = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 16 9" preserveAspectRatio="xMidYMid slice"><defs><linearGradient id="g" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="%239c27b0"/><stop offset="1" stop-color="%236367f1"/></linearGradient></defs><rect width="16" height="9" fill="url(%23g)"/><text x="8" y="5" fill="white" font-family="Arial, Helvetica, sans-serif" font-size="1.6" text-anchor="middle" dominant-baseline="middle">Video</text></svg>';
...
const NETLIFY_CONFIG_API_BASE = 'https://toddler-roku.netlify.app/api/config';
...
function resolveTabSections(tab) {
    if (!tab) return [];

    // Prefer hardcoded layout if we know this tab ID
    const definitionSections = TAB_DEFINITIONS[tab.id]?.sections;
    if (Array.isArray(definitionSections) && definitionSections.length) {
        return [...definitionSections];
    }

    // Allow explicit sections array in config
    if (Array.isArray(tab.sections) && tab.sections.length) {
        return [...tab.sections];
    }

    // Heuristics: quickLaunch/buttons imply the Roku/apps layout
    if (Array.isArray(tab.quickLaunch) || Array.isArray(tab.buttons)) {
        return [...(TAB_DEFINITIONS.apps?.sections || [])];
    }

    // Fallback to the remote layout so we never render an empty page
    return [...(TAB_DEFINITIONS.remote?.sections || [])];
}
...
function getTabsForRendering() {
    ...
    const allowedTabIds = new Set(Object.keys(TAB_DEFINITIONS));

    // If we have a loaded config, use it
    if (tabsConfig && Array.isArray(tabsConfig.tabs)) {
        const safeTabs = tabsConfig.tabs.filter(tab => allowedTabIds.has(tab.id));
        tabs = safeTabs.map(tab => ({
            ...
            // Resolve to a known section list so custom tab IDs still render content
            sections: resolveTabSections(tab)
        }));
    } else {
    ...
    // Limit web builds to Roku-focused tabs
    tabs = filterTabsForEnvironment(tabs);

    // Guarantee at least the Roku remote tab is present
...
function getActiveTabId() {
    ...
    if (!window._activeTabId || !availableTabs.some(tab => tab.id === window._activeTabId)) {
        // Default to the first available tab from config to avoid blank states
        window._activeTabId = availableTabs[0].id || 'remote';
    }
    return window._activeTabId;
}
...
function applyTabVisibility(activeTabId, availableTabs) {
    ...
    // Only consider sections that actually exist in the managed list
    let visibleSections = new Set(
        (activeTab?.sections || []).filter(id => TAB_MANAGED_SECTION_IDS.includes(id))
    );
    // If a custom tab provides no known sections, fall back to the remote layout
    if (visibleSections.size === 0) {
        visibleSections = new Set(TAB_DEFINITIONS.remote.sections || []);
    }
...
function normalizeQuickLaunchItem(item) {
    ...
    // Default type to youtube when a videoId is present
    if (!normalized.type && normalized.videoId) {
...
function applyToddlerContent(data) {
    ...
    console.log('[Config] Applying toddler content, tabs:', data?.tabs?.length);
    ...
    const remoteButtons = filterEnabledButtons(remoteTab?.buttons);
    const appsButtons = filterEnabledButtons(appsTab?.buttons);
    ...
    try {
        ...
        // Older webviews may not support Event constructor
        const evt = document.createEvent('Event');
        evt.initEvent('tabs-updated', true, true);
...
function renderToddlerButtons(remoteButtons = [], appsButtons = [], quickLaunch = []) {
    const layout = document.getElementById('toddlerControlsInner');
    ...
    if (!remoteColumn || !layout) return;

    remoteColumn.innerHTML = '';

    // Hide quick column on the remote tab to keep YouTube/quick launch content in the Apps tab only
    if (quickColumn) {
        quickColumn.innerHTML = '';
        quickColumn.classList.add('hidden');
    }
    layout.className = 'grid gap-6'; // single-column layout for remote controls
...
function getThumbnailSources(config) {
    const sources = [];
    if (config.thumbnail) {
        sources.push(config.thumbnail);
    }

    const rawId = config.videoId || config.launchItem?.videoId || '';
    const isYoutube =
        config.type === 'youtube' ||
        config.launchItem?.type === 'youtube' ||
        rawId.length > 0;
    const videoId = rawId || (config.id && config.id.startsWith('yt-') ? config.id.slice(3) : '');
    ...
    // Final built-in placeholder to guarantee something shows
    sources.push(
        'data:image/svg+xml;base64,'
    );

    return sources.filter(Boolean);
}

function applyThumbnailWithFallback(img, sources) {
    const list = Array.isArray(sources) ? sources.slice() : [];
    list.push(FALLBACK_THUMB_PLACEHOLDER);
    ...
    const tryNext = () => {
        ...
        // Set a timeout as fallback in case error event doesn't fire
    ...
    const onSuccess = () => {
        ...
        // Check if this is YouTube's 404 placeholder (120x90px gray image with TV icon)
        // YouTube returns HTTP 200 with a placeholder instead of 404
    ...
    img.onerror = tryNext; // Fallback for older behavior

    tryNext();
}

function filterEnabledButtons(buttons) {
    if (!Array.isArray(buttons)) return [];
    return buttons.filter(btn => btn.enabled !== false);
}
...
function createQuickButtonElement(config) {
    ...
    const hasThumbnail = Boolean(
        config.thumbnail ||
        (isQuickLaunch && (config.launchItem?.videoId || config.videoId))
    );
    ...
    if (hasThumbnail) {
        buttonEl.style.background = 'linear-gradient(135deg, rgba(99,102,241,0.35), rgba(147,51,234,0.35))';
    }
    ...
    if (hasThumbnail) {
        ...
        // Apply fallback AFTER adding to DOM to ensure onerror fires correctly
        const thumbSources = getThumbnailSources(config);
        applyThumbnailWithFallback(img, thumbSources);
    } else {
        const iconSpan = document.createElement('span');
...
function renderLaunchGridSection(sectionId, gridId, items, options = {}) {
    ...
    launches.forEach(item => {
        const button = document.createElement('button');
        button.className = 'group relative overflow-hidden rounded-3xl shadow-lg transition hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-white/40 active:scale-[0.98] touch-manipulation select-none aspect-[16/9]';
        button.type = 'button';
        ...
        const img = document.createElement('img');
        button.style.background = 'linear-gradient(135deg, rgba(99,102,241,0.35), rgba(147,51,234,0.35))';
        ...
        // Apply fallback AFTER adding to DOM to ensure onerror fires correctly
        const thumbSources = getThumbnailSources({ ...item, launchItem: item });
        applyThumbnailWithFallback(img, thumbSources);
    });

    // Always show quick launch when items exist
    section.classList.remove('hidden');
}
...
function showToast(message, type = 'info', duration = 3200) {
    ...
    toast.className = `pointer-events-auto flex items-center gap-3 rounded-2xl px-5 py-3 text-sm font-semibold shadow-xl ${variant.classes}`;
...
// Expose settings handlers for inline event attributes
if (typeof window !== 'undefined') {
    window.startSettingsHold = startSettingsHold;
    window.stopSettingsHold = stopSettingsHold;
